...
function btn_like_obj_click(source) {
    var sender = $('#' + source.id);
    var objId = sender.next('input[name="SharedObjectId"]').val();
    var allObjLikeButtons = $("[id^=btn_like_object_" + objId + "]");
    allObjLikeButtons.each(function () {
        var has_liked_shared_obj = ($(this).parent().parent().find('div.actions_summery').find('input[name="has_liked_shared_obj"]').val() == 'true' ? true : false);
        if (!has_liked_shared_obj) {
            $(this).text($('input#w_unlike').val());
            var summ_a = $(this).parent().parent().find('div.actions_summery').find('a');
            var likes_count = Number($(this).parent().parent().find('div.actions_summery').find('input[name="likes_count"]').val());
            likes_count++;
            $(this).parent().parent().find('div.actions_summery').find('input[name="likes_count"]').val(likes_count);
            $(this).parent().parent().find('div.actions_summery').find('input[name="has_liked_shared_obj"]').val(!has_liked_shared_obj);
            summ_a.text(get_likes_text(likes_count, !has_liked_shared_obj));
            summ_a.removeClass("nolike").attr('onclick', 'btn_obj_likes_click();');
        }
        else {
            ...
            summ_a.text(get_likes_text(likes_count, !has_liked_shared_obj));
            if (likes_count == 0) {
                summ_a.addClass("nolike").attr('onclick', '');
            }
        }
    });
}
...
function btn_obj_share_click() {
    var dialog = new Dialog(700, 500, '<div class="loading_dialog"></div>');
    dialog.Show();
}
