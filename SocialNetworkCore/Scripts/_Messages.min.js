$(function () {
    ...
    $('div#messages a.new_msg').live("click", function () {
        var dialog = new Dialog(550, 450, '<div class="loading_dialog"></div>');
        dialog.Show();
    });
});
/* ************************* AJAX Functions *************************** */
...
function LoadConversationSuccess(data) {
    if (data.Done) {
        ...
        $('div#conv_reply .msg_sender').find('input[name="OpponentId"]').eq(0).val(data.ConversationId);
        toggle_msg_sender($('div#conv_reply .msg_sender'));
        //******** Update Unreads Count:
        $('div#messages #hid_updated_unreads_count').val(data.UpdatedUnreadsCount);
        // call this func from _HeadersHeadTags.js
        show_msg_bubble(data.UpdatedUnreadsCount);
    }
    else {
        $('div#conv_messages_in').html('<div class="error_message">' + data.Errors[0] + '</div>');
    }
}
