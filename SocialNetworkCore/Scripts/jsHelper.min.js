...
function split(st, char) {
    var list = st.split(char);
    var res = new Array();
    for (var i = 0; i < list.length; i++) {
        if ($.trim(list[i]).length > 0)
            res.push(list[i]);
...
function add_css(filename) {
    ...
    if (!is_css_added(filename)) {
        ...
        lnk.href = '/App_Themes/' + $('#app_theme').val() + '/' + $('#app_lang').val() + '/' + filename;
        lnk.rel = 'stylesheet';
        lnk.type = 'text/css';
...
function show_error_dialog(txt) {
    var html =
        '<div>' +
            '<div class="dialog_title">' +
                '<a class="dialog_close" onclick="HideDialog();"></a>' +
                '<h2>' + $('#gw_ErrorDialogTitle').val() + '</h2>' +
                '<div class="clear"></div>' +
            '</div>' +
            '<div class="dialog_view_in">' +
                '<div class="dialog_message_content">' +
                    '<div class="dialog_message_icon error_icon"></div>' +
                    '<div class="dialog_message_text">' +
                        txt +
                    '</div>' +
                    '<div class="clear"></div>' +
                '</div>' +
                '<div class="dialog_actions">' +
                    '<a class="cancel" onclick="HideDialog();">' + $('#gw_ErrorDialogClose').val() + '</a>' +
                    '<div class="clear"></div>' +
                '</div>' +
                '<div class="clear"></div>' +
            '</div>' +
        '</div>';
    var dialog = new Dialog(500, 350, html);
    dialog.Show();
}

function show_confirm_dialog(title, content, callbackFuncName) {
    var html =
        '<div id="my_confirm_dialog">' +
            '<div class="dialog_title">' +
                '<a class="dialog_close" onclick="HideDialog();"></a>' +
                '<h2>' + title + '</h2>' +
                '<div class="clear"></div>' +
            ...
            '<div class="dialog_view_in">' +
                '<div class="dialog_message_content">' +
                    '<div class="dialog_message_icon warning_icon"></div>' +
                    '<div class="dialog_message_text">' +
                        content +
                    '</div>' +
                    '<div class="clear"></div>' +
                '</div>' +
                '<div class="dialog_actions">' +
                    '<a class="cancel" onclick="HideDialog();">' + $('#gw_Cancel').val() + '</a>' +
                    '<a class="okey" onclick="' + callbackFuncName + '();HideDialog();">' + $('#gw_OK').val() + '</a>' +
                    '<div class="clear"></div>' +
                '</div>' +
                '<div class="clear"></div>' +
            '</div>' +
