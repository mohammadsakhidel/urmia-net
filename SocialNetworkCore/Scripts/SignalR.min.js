$(function () {
    ...
    hub.client.chatMessageDelivered = function (signal) {
        chat_message_delivered(signal);
    }
...
function update_chat(signal) {
    ...
    if (is_dlg_open) {
        ...
        $.connection.socialNetHub.server.setMessageAsRead(msg, true);
...
function chat_message_delivered(signal) {
    var is_dlg_open = is_session_open(signal.SessionId);
    if (is_dlg_open) {
        var msgId = signal.MessageId;
        $('#chat_msg_del_' + msgId).removeClass('hide');
    }
}
